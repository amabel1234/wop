const API_BASE = "";
const SESSION_KEY = "nixx_admin_token";
const DISCORD_URL = "https://discord.gg/e4yQCm2u";
...
function setToken(token) {
  ...
  const hidden = document.querySelector("#adminToken");
...
function showLogin(message = "") {
  ...
  if (loginToken) { loginToken.value = ""; setTimeout(() => loginToken.focus(), 50); }
}
...
function showAdmin() {
  ...
  const hidden = document.querySelector("#adminToken");
  if (hidden) hidden.value = getToken();
}

async function verifyAdminToken(token) {
  const response = await fetch(`${API_BASE}/api/role`, {
    method: "GET",
    headers: { Authorization: `Bearer ${token}` },
    cache: "no-store"
  });
  const data = await readApiResponse(response);
  if (!response.ok || data.role !== "admin") throw new Error("Token admin tidak valid.");
  return data;
}
...
async function loginAdmin(token) {
  ...
  currentRole = "admin";
  showAdmin();
  if (roleBadge) roleBadge.textContent = "Role: ADMIN";
  initAdminPanel();
}
...
// ===== NIXX ADMIN PANEL AUTH + SIDEBAR =====
function setActivePage(page) {
  document.querySelectorAll(".admin-page").forEach((el) => {
    el.hidden = el.dataset.page !== page;
  });
  document.querySelectorAll(".admin-nav button[data-page-target]").forEach((btn) => {
    btn.classList.toggle("active", btn.dataset.pageTarget === page);
  });
  if (adminSidebar && window.innerWidth <= 900) adminSidebar.classList.remove("open");
  window.scrollTo({ top: 0, behavior: "smooth" });

  if (page === "licenses") loadLicenses();
  if (page === "announcement") loadAnnouncement();
  if (page === "products") loadProductsAdmin();
...
async function loadDashboard() {
  const token = getToken();
  if (!token) return;
  const stats = { licenses: [], products: [], scripts: [] };
  try {
    const [licenseRes, productRes, scriptRes, announcementRes] = await Promise.all([
      fetch(`${API_BASE}/api/licenses/list`, { headers: { Authorization: `Bearer ${token}` }, cache: "no-store" }),
      fetch(`${API_BASE}/api/products?ts=${Date.now()}`, { cache: "no-store" }),
      fetch(`${API_BASE}/api/scripts?ts=${Date.now()}`, { cache: "no-store" }),
      fetch(`${API_BASE}/api/announcement?ts=${Date.now()}`, { cache: "no-store" })
    ]);
    const [licenses, products, scripts, announcement] = await Promise.all([
      readApiResponse(licenseRes), readApiResponse(productRes), readApiResponse(scriptRes), readApiResponse(announcementRes)
    ]);
    stats.licenses = Array.isArray(licenses.licenses) ? licenses.licenses : [];
    stats.products = Array.isArray(products.products) ? products.products : [];
    stats.scripts = Array.isArray(scripts.scripts) ? scripts.scripts : [];
    const active = stats.licenses.filter(x => String(x.status).toLowerCase() === "active").length;
    document.querySelector("#statLicenses").textContent = stats.licenses.length;
    document.querySelector("#statActive").textContent = active;
    document.querySelector("#statProducts").textContent = stats.products.length;
    document.querySelector("#statScripts").textContent = stats.scripts.length;
    const a = announcement.announcement || {};
    document.querySelector("#dashboardAnnouncement").textContent = a.enabled && a.text ? `Aktif — ${a.text}` : "Tidak ada announcement aktif.";
  } catch (error) {
    ["statLicenses","statActive","statProducts","statScripts"].forEach(id => { const el=document.querySelector(`#${id}`); if(el) el.textContent="!"; });
    const el=document.querySelector("#dashboardAnnouncement"); if(el) el.textContent=error.message || "Gagal memuat dashboard.";
  }
}

function initAdminPanel() {
  document.querySelectorAll(".admin-nav button[data-page-target]").forEach((button) => {
    button.onclick = () => setActivePage(button.dataset.pageTarget);
  });
  mobileMenu?.addEventListener("click", () => adminSidebar?.classList.toggle("open"));
  sidebarLogout?.addEventListener("click", () => { showLogin(); });
  ...
  document.querySelector("#sessionRole").textContent = "ADMIN";
  document.querySelector("#settingsSession").textContent = "Active";
...
loginForm?.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    showLogin(error.message || "Token admin tidak valid.");
  } finally {
    loginButton.disabled = false;
    loginButton.textContent = "Masuk ke Admin Panel ↗";
  }
});
...
(async function bootAdmin() {
  ...
  try {
    ...
    showLogin("Session admin sudah tidak valid. Silakan login lagi.");
  }
})();
