...
(function initNixxManualProducts(){
  ...
  async function load(){
    ...
    try{const r=await fetch(`/api/products?ts=${Date.now()}`,{cache:"no-store"}); if(!r.ok)return; const d=await r.json(); items=Array.isArray(d.products)?d.products:[]; render();}catch(_){}finally{loading=false;}
  }
